...
<form action="{{ route('transaction.store') }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="mb-3">
    <label for="name" class="form-label">Pilih Merek Mobil</label>
    <select class="form-select" id="status" name="car_id">
      @foreach($cars as $car)
      @if($car->status == '1') {{-- Memeriksa apakah mobil tersedia --}}
      <option {{ (old('car_id') == $car->id ? 'selected' : '') }} value="{{ $car->id }}">{{ $car->merek }}</option>
      @endif
      @endforeach
    </select>
    @error('car_id')
